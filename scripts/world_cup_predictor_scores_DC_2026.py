...
#   5. API/manual completed group-stage results
...
#
# Run:
#   python world_cup_predictor_2026.py
# ------------------------------------------------------------
...
# Ideally estimate this from historical data.
...
# Score matrix truncation. 10 is usually plenty for football.
...
def fetch_played_matches_from_api_football(groups):
    ...
    if not api_key:
        print("No API_FOOTBALL_KEY found. Using manual results only.")
        return []
...
def get_played_matches(groups):
    if RESULT_SOURCE not in {"api", "manual", "both"}:
        raise ValueError(
            'RESULT_SOURCE must be one of: "api", "manual", or "both".'
        )
    ...
    if RESULT_SOURCE in {"manual", "both"}:
...
def expected_goals(team, opponent, elo):
    ...
    Replace these coefficients with fitted coefficients from your
    Elo-goals estimation script if desired.
    """
...
def score_probability_matrix(mu_a, mu_b, rho=0.0, max_goals=10):
    ...
    If rho=0, this is just independent Poisson.
    If rho!=0, this applies Dixon-Coles low-score correction.
    """
    scores = []
    probs = []
...
def simulate_score(team_a, team_b, elo):
    """
    Simulate a 90-minute scoreline.

    If USE_DIXON_COLES is True, samples from the Dixon-Coles corrected
    score probability matrix. Otherwise, samples from independent Poisson.
    """
    if USE_DIXON_COLES:
...
def predict_group_fixture_exact(team_a, team_b, elo):
    """
    Predict one fixture exactly from the score probability matrix.
    This avoids Monte Carlo noise for fixture-level probabilities.
    """
    scores, probs = get_score_matrix_for_match(team_a, team_b, elo)
...
def main():
    ...
    print(f"Loaded {len(results):,} historical matches before {CUTOFF_DATE.date()}.")

    print("\nFetching completed World Cup group-stage matches...")
    print(f"Result source: {RESULT_SOURCE}")

    played_matches = get_played_matches(GROUPS)
    ...
    if UPDATE_ELO_WITH_PLAYED_MATCHES:
        print("\nAdding completed World Cup group matches to Elo data...")
        results_for_elo = append_played_matches_to_results(results, played_matches)
    else:
        print("\nUsing pre-tournament Elo only...")
        results_for_elo = results
